    ...
    if (configFile) {
        ...
        if (!providerName.empty() && providerName != "local") {
            ...
            if (provider) {
                ...
                if (providerName == "s3") {
                    auto s3cfg = Json::Object();
                    s3cfg.objVal["endpoint"]   = Json::String(cfg["s3_endpoint"].str());
                    s3cfg.objVal["bucket"]     = Json::String(cfg["s3_bucket"].str());
                    s3cfg.objVal["region"]     = Json::String(cfg["s3_region"].str());
                    s3cfg.objVal["key_id"]     = Json::String(cfg["s3_key_id"].str());
                    s3cfg.objVal["secret_key"] = Json::String(cfg["s3_secret_key"].str());
                    initPath = Json::Stringify(s3cfg);
                }
